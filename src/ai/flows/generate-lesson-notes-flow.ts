...
'use server';
/**
 * @fileOverview This file defines a Genkit flow for generating lesson notes based on a topic, key points, desired format, student audience, and language style.
 *
...
export async function generateLessonNotes(
  ...
  console.log(`[${flowName}] Called with topic: ${input.lessonTopic}, format: ${input.noteFormat}, audience: ${input.studentAudience || 'general'}, language: ${input.languageOutputStyle}`);
...
-   **For Technical Drawing or Engineering Drawing topics (e.g., Orthographic Projection, Isometric Drawing, Dimensioning, CAD basics - always tailor to {{{studentAudience}}} and apply {{{languageOutputStyle}}} if 'simplified-english' is chosen):**
...
-   **For All Other Technical, Vocational, and Theoretical Subjects (e.g., Food Technology, Water Engineering, ICT, Plumbing, Electrical Installation, Humanities, Social Sciences, Arts, Conceptual Sciences, etc. - always tailor to {{{studentAudience}}} and apply {{{languageOutputStyle}}} if 'simplified-english' is chosen):**
...
Please generate the lesson notes now. Ensure the content is rich and covers the topic and key points in significant depth according to the guidelines above, especially tailoring for the {{{studentAudience}}} and adhering to the requested {{{languageOutputStyle}}}.
...
Avoid superficial or overly brief content. Aim for notes that a trainer can rely on for a substantial lesson. The explanation should be as human and clear as possible, making complex topics accessible to the specified {{{studentAudience}}} and in the chosen language style.
