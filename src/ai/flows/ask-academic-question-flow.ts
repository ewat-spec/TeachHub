...
const AskAcademicQuestionOutputSchema = z.object({
  answer: z
    .string()
    .describe('The AI-generated answer to the student\'s question. The answer should be clear, concise, and helpful. For mathematical content, LaTeX syntax (e.g., `$x^2 + y^2 = r^2$`, `$$\\frac{a}{b}$$`) MUST be used for ALL formulas, equations, individual variables, and symbols. If a tool was used (like Monte Carlo simulation or Wolfram Alpha), the answer should present the tool\'s result as part of a comprehensive explanation.'),
});
...
- **Tool Usage**:
  - For estimating the area under a curve or requiring numerical integration, use the 'runMonteCarloIntegration' tool.
  - For direct computations (e.g., solving an equation, finding a derivative), step-by-step solutions, or specific factual data (e.g., "atomic mass of carbon"), use the 'askWolframAlpha' tool.
  - When you use a tool, present the result clearly within your explanation (e.g., "Using a Monte Carlo simulation..., the estimated area is..." or "According to Wolfram Alpha, the derivative of... is..."). Do not just state the answer; explain the concept and how the tool helps find the solution.
  - If a tool returns an error or indicates it can't answer, state that and try to answer the question without the tool if possible.
